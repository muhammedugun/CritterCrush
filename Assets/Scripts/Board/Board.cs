...
namespace Match3
{
    ...
    public class Board : MonoBehaviour
    {
        ...
        void ClearAllGems()
        {
            ...
            {
                // Hücredeki gem nesnesi varsa yok et
                ...
                {
                    // Görsel objeyi yok et
                    Destroy(cell.ContainingGem.gameObject);
                    // Mantıksal olarak gemi temizle
            ...
            FindAllPossibleMatch();
            // Hücrelerde hala olası hamle yoksa
        ...
        void GenerateBoard()
        {
            ...
            // Hücrelerin içeriğini for döngüleri ile kontrol ediyor
            for (int y = m_BoundsInt.yMin; y <= m_BoundsInt.yMax; ++y)
            {
                for (int x = m_BoundsInt.xMin; x <= m_BoundsInt.xMax; ++x)
                {
                    ...
                    // idx kordinatında herhangi bir hücre yoksa ya da var olan hücrede herhangi bir gem varsa
                    if(!CellContent.TryGetValue(idx, out var current) || current.ContainingGem != null)
                        continue; // döngünün bu adımını atlayarak sonraki kordinatı kontrol et.
